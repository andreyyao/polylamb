use compiler::system_f::parse::{parse_val_expr, parse_decl};
...
#[test]
fn check_exprs() {
    for s in LITERALS { assert_matches!(raw_expr_of(s), RawExpr::Con {..}) }
    for s in BINOPS { assert_matches!(raw_expr_of(s), RawExpr::Binop {..}) }
    for s in IFS { assert_matches!(raw_expr_of(s), RawExpr::If {..}) }
}


#[test]
fn check_decl() {
    for s in DECLS { assert_matches!(parse_decl(s), Result::Ok(..)) }
}