...
\begin{figure}[h]
  ...
  \begin{tabular}{l p{\ColOne} p{\ColTwo} p{\ColThree} p{\ColFour}}
    ...
    \newrow{\kwm{\lambda} \gm{pat^{\geq 1}}\kwt{.} \gm{e}}{lambda abstraction}
    \newrow{\kwm{\Lambda} \gm{tid^{\geq 1}}\kwt{.} \gm{e}}{type abstraction}
    ...
    \hline
    \row{pat}{\gm{eid} \kwt{:} \gm{\tau}}{type-annotated variable}
...
\begin{figure}[h]
  ...
  \begin{tabular}{l p{\ColOne} p{\ColTwo} p{\ColThree} p{\ColFour}}
    \hline
    \row{\tau}{\gm{\tau}}{types}
    \newrow{\gm{tid}}{type identifier}
    \newrow{\kwt{(}\gm{\tau}\kwt{)}}{parenthesized}
    \newrow{\gm{\tau_1} \kwt{->} \gm{\tau_2}}{arrow types}
    \newrow{\kwm{\,\forall} \gm{tid^{\geq 1}}\kwt{.} \gm{\tau}}{universal types}
...
\subsection*{Notes}
Multiple argument \kwm{\lambda}'s, \kwm{\Lambda}'s, and \kwm{\forall}'s are syntactic sugar for nested versions. For instance,
\begin{center}
  $\lambda$ \texttt{x:Int y:Int.} \texttt{x} \texttt{+} \texttt{y}
   $\stackrel{\text{def}}{=}$
  $\lambda$ \texttt{x:Int.} $\lambda$ \texttt{y:Int.} \texttt{x} \texttt{+} \texttt{y}
\end{center}
